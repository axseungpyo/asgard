#!/bin/bash
# install.sh — Asgard 글로벌 설치 스크립트
#
# 로컬:  bash install.sh
# 원격:  curl -fsSL https://raw.githubusercontent.com/USER/asgard/main/install.sh | bash
#
# 설치 내용:
#   1. Skills -> ~/.claude/skills/ (모든 Claude Code 세션에서 사용 가능)
#   2. 코어 파일 -> ~/.asgard/ (AGENTS.md, 템플릿, 스크립트)
#   3. asgard CLI -> ~/.asgard/bin/asgard (PATH 등록)
...
if $REMOTE; then
    ...
    ASGARD_SRC="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    echo -e "${CYAN}모드: 로컬 (${ASGARD_SRC})${NC}"
fi
echo ""

# ─── 1. 디렉토리 생성 ───
echo -e "${CYAN}[1/5] 디렉토리 준비...${NC}"
mkdir -p "$ASGARD_HOME/bin"
mkdir -p "$ASGARD_HOME/templates"
mkdir -p "$ASGARD_HOME/scripts"
...
cp "$ASGARD_SRC/.claude/commands/init.md" "$CLAUDE_DIR/commands/init.md"
echo -e "   ${GREEN}✓${NC} /init 커맨드"

# ─── 3. 코어 파일 설치 ───
echo -e "${CYAN}[3/5] Asgard 코어 파일 설치 (~/.asgard/)...${NC}"

cp "$ASGARD_SRC/AGENTS.md" "$ASGARD_HOME/templates/AGENTS.md"
...
- artifacts/handoff/: Rune/Saga 교환소
...
cat > "$ASGARD_HOME/templates/settings.json" << 'EOF'
{
  "env": {
...
# 설치된 버전 저장
if [ -f "$ASGARD_SRC/VERSION" ]; then
    cp "$ASGARD_SRC/VERSION" "$ASGARD_HOME/VERSION" 2>/dev/null || true
fi
...
cat > "$ASGARD_HOME/bin/asgard" << 'ASGARD_CLI'
#!/bin/bash
# asgard — Asgard 프로젝트 관리 CLI
ASGARD_GITHUB_USER="axseungpyo"
ASGARD_GITHUB_REPO="asgard"
...
usage() {
    local ver=""
    [ -f "$ASGARD_HOME/VERSION" ] && ver=" v$(cat "$ASGARD_HOME/VERSION")"
    echo -e "${BOLD}Asgard CLI${ver}${NC}"
    echo ""
    echo "Usage: asgard <command> [options]"
    echo ""
    echo "Commands:"
    echo "  new <name>    새 Asgard 프로젝트 생성"
...
cmd_new() {
    ...
    echo -e "${CYAN}Asgard 프로젝트 초기화: ${BOLD}$name${NC}"
    echo ""
    ...
    if [ ! -f "CLAUDE.md" ]; then
        cp "$ASGARD_HOME/templates/CLAUDE.md" "CLAUDE.md"
        echo -e "   ${GREEN}✓${NC} CLAUDE.md"
    else
        echo -e "   ${YELLOW}~${NC} CLAUDE.md (이미 존재, 유지)"
    fi

    cp "$ASGARD_HOME/templates/AGENTS.md" "AGENTS.md"
    echo -e "   ${GREEN}✓${NC} AGENTS.md"

    cp "$ASGARD_HOME/templates/settings.json" ".claude/settings.json"
    echo -e "   ${GREEN}✓${NC} .claude/settings.json"

    cp "$ASGARD_HOME/scripts/delegate-gemini.sh" "scripts/delegate-gemini.sh"
...
EOF
    ...
    if [ ! -f "shared/context.md" ]; then
        sed "s/{DATE}/$today/" "$ASGARD_HOME/templates/context.md" > "shared/context.md"
...
cmd_update() {
    echo -e "${CYAN}Skills 업데이트 중 (GitHub: ${ASGARD_GITHUB_USER}/${ASGARD_GITHUB_REPO})...${NC}"

    if ! command -v curl &>/dev/null; then
        echo -e "${RED}오류: curl이 필요합니다.${NC}"; exit 1
    fi

    local base_url="https://raw.githubusercontent.com/${ASGARD_GITHUB_USER}/${ASGARD_GITHUB_REPO}/${ASGARD_GITHUB_BRANCH}"
    ...
    # 버전 업데이트
    curl -fsSL "$base_url/VERSION" -o "$ASGARD_HOME/VERSION" 2>/dev/null || true
...
cmd_doctor() {
    local ver=""
    [ -f "$ASGARD_HOME/VERSION" ] && ver=" v$(cat "$ASGARD_HOME/VERSION")"
    echo -e "${BOLD}Asgard 설치 진단${ver}${NC}"
    echo ""
...
chmod +x "$ASGARD_HOME/bin/asgard"
echo -e "   ${GREEN}✓${NC} ~/.asgard/bin/asgard"
...
PATH_LINE='export PATH="$HOME/.asgard/bin:$PATH"'
if [ -n "$SHELL_RC" ]; then
    if ! grep -q ".asgard/bin" "$SHELL_RC" 2>/dev/null; then
        { echo ""; echo "# Asgard CLI"; echo "$PATH_LINE"; } >> "$SHELL_RC"
...
export PATH="$HOME/.asgard/bin:$PATH"

echo ""
echo -e "${GREEN}${BOLD}Asgard 설치 완료!${NC}"
echo ""
echo -e "터미널을 재시작하거나:"
[ -n "$SHELL_RC" ] && echo -e "  ${CYAN}source $SHELL_RC${NC}"
echo ""
echo -e "새 프로젝트:"
echo -e "  ${CYAN}asgard new my-project${NC}"
echo -e "  ${CYAN}asgard doctor${NC}"
