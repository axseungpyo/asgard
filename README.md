...
Claude(Odin)를 중심으로 GPT Codex(Brokkr), Gemini(Heimdall), Loki(Image Gen)를 협업시키는
...
(Codex)   (Gemini)   (Image Gen)
코드 구현   비전 분석   이미지 생성
테스트/빌드  OCR/리서치  편집/디자인
```
...
| **Loki** | Image Gen API | 이미지 생성/편집 전문 (Google AI Imagen) |
...
Loki:      Sketch[Fast] / Canvas[Standard]
```
...
Asgard 에이전트의 실시간 모니터링 대시보드.

```bash
# 실행
cd src/yggdrasil && npm install && npm run dev
# 또는
asgard dashboard
```

http://localhost:7777 에서 접속.

### 기능

| 탭 | 기능 |
|----|------|
| **Overview** | 에이전트 상태 카드 + Quick Actions + Chronicle + 실시간 로그 |
| **Terminals** | 에이전트별 터미널 뷰 (4-agent 로그 분리, 에이전트 컬러 바) |
| **Flow** | TP→Agent→RP 파이프라인 시각화 + DAG 의존성 그래프 |
| **Stats** | 완료율, 에이전트별 분포, 일별 통계, 실행 이력 |
| **Skills** | Skill 카탈로그 + SKILL.md 문서 뷰어 + 실행 UI |

### 추가 기능

- **Dark/Light 모드** — 헤더 토글 또는 Settings에서 전환 (Warm Canvas light 테마)
- **Settings 패널** — Appearance, Notifications (에이전트별), Connection, API Keys (12 프로바이더)
- **브라우저 알림** — 에이전트 완료/차단 시 자동 알림
- **토큰 인증** — Bearer 토큰 기반 API/WebSocket 보호 (`YGGDRASIL_AUTH=false`로 비활성화)
...
| Backend | Express 5, WebSocket (ws), Pino 로깅 |
| 파일 감시 | Chokidar (INDEX.md, 로그, PID 실시간 추적) |
| 실시간 통신 | WebSocket (`/ws/logs`, `/ws/status`) + 하트비트 |
| 테스트 | Vitest (34+ 단위 테스트) |
...
src/yggdrasil/
├── server/
│   ├── index.ts          # Express + Next.js + WebSocket 통합 서버
│   ├── routes.ts         # REST API (/api/status, /api/chronicle, /api/metrics, /api/skill)
│   ├── auth.ts           # Bearer 토큰 인증 미들웨어
│   ├── watcher.ts        # 파일 시스템 감시 (로그, INDEX.md, PID)
│   ├── parser.ts         # INDEX.md 마크다운 테이블 파서
│   ├── agents.ts         # 에이전트 상태 추론 (PID + 태스크 기반)
│   ├── metrics.ts        # 에이전트 메트릭 수집 (성공률, 실행 시간)
│   ├── dependency.ts     # TP 의존성 그래프 (DAG, 순환 감지)
│   └── logger.ts         # Pino 구조화 로깅
├── dashboard/
│   ├── app/
│   │   ├── page.tsx          # 메인 대시보드 (5탭)
│   │   ├── layout.tsx        # 레이아웃 + 테마
│   │   └── globals.css       # CSS 변수 기반 테마 시스템
│   ├── components/
│   │   ├── Header.tsx        # 상단 바 (연결 상태 + 테마 토글 + 설정)
│   │   ├── ThemeToggle.tsx   # Dark/Light 모드 토글
│   │   ├── ApiKeysModal.tsx  # 종합 Settings 패널 (4탭)
│   │   ├── AgentCard.tsx     # 에이전트 상태 카드
│   │   ├── Chronicle.tsx     # 태스크 목록 (검색/필터)
│   │   ├── LiveLog.tsx       # 실시간 로그 스트림
│   │   ├── TerminalPanel.tsx # 에이전트별 터미널 뷰
│   │   ├── SkillsPanel.tsx   # Skill 카탈로그 + 문서 뷰어
│   │   ├── QuickActions.tsx  # 빠른 실행 (Status, Validate)
│   │   ├── FlowView.tsx      # TP→Agent→RP 파이프라인
│   │   ├── DependencyView.tsx # DAG 의존성 그래프
│   │   ├── StatsPanel.tsx    # 통계 대시보드
│   │   ├── MetricsPanel.tsx  # 메트릭 차트
│   │   └── DocViewer.tsx     # TP/RP 마크다운 뷰어
│   └── lib/
│       ├── types.ts      # TypeScript 타입
│       ├── constants.ts  # 에이전트 설정 (SSoT)
│       ├── theme.ts      # 테마 전환 로직
│       ├── auth.ts       # 인증 유틸리티
│       └── websocket.ts  # WebSocket 훅 (자동 재연결)
└── package.json
```
...
asgard dashboard        # Yggdrasil 대시보드 실행
```
...
├── scripts/
│   ├── delegate-codex.sh      # Brokkr 실행 래퍼 + Watchdog
│   ├── delegate-gemini.sh     # Heimdall 실행 래퍼 + Watchdog
│   ├── delegate-loki.sh       # Loki 실행 래퍼
│   ├── rollback.sh            # git revert 기반 롤백
│   └── archive-done.sh        # done 태스크 자동 아카이빙
└── src/
    └── yggdrasil/             # 에이전트 모니터링 대시보드
```
